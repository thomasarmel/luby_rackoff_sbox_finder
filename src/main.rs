...
fn main() {
    ...
    let mut count = 0;
    for i in 0..(1 << SBOX_SIZE) {
        let input_bits: [bool; SBOX_SIZE] = unsigned_to_bool_array::<SBOX_SIZE>(i);
        ...
        let output_unsigned = bool_array_to_unsigned::<SBOX_SIZE>(round_7);
        if round_4[0] == input_bits[0] ^ input_bits[5] ^input_bits[6] ^ input_bits[7] ^ input_bits[8] {
            count += 1;
        }
